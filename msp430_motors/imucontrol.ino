...
void setup() {
    ...
    Serial.print(mpu.testConnection() ? '1' : '0');
    Serial.print(mag.testConnection() ? '1' : '0');
    Serial.print(baro.testConnection() ? '1' : '0');
    Serial.println();
...
void loop() {
    ...
    if (Serial.available() > 0) {
      ...
      if (cmd >= '0' && cmd <= '9') {
        uint8_t speed = (uint8_t)(cmd - '0') * 26;
        setMotors(speed, speed, speed, speed);
      } 
      else if (cmd == ' ') setMotors(0, 0, 0, 0);
      else if (cmd == 'Q') setMotors(30, 0, 0, 0);
      else if (cmd == 'W') setMotors(0, 30, 0, 0);
      else if (cmd == 'E') setMotors(0, 0, 30, 0);
      else if (cmd == 'R') setMotors(0, 0, 0, 30);
    }
